...
const findCar = (car) => {
  ...
  Vehicleshowroom.find({
    $or: [{ modelNumber: search }, { enginePower: search }],
 ...
 */
const removeCar = (_id) => {
  Vehicleshowroom.remove({ _id }).then((vehicleshowroom) => {
 ...
 */
const listOfCar = () => {
  Vehicleshowroom.find().then((vehicleshowrooms) => {
  ...
  Vehicleshowroom.find().then((vehicleshowrooms) => {
    vehicleshowrooms.map((vehicle) => {
      if (vehicle.vehicleType === "sports") {
        ...
        console.info(
          "Number of visitors:",
          Math.floor(Math.random() * (60 - minimumRange + 1) + minimumRange)
        );
      }
      if (vehicle.vehicleType === "normal") {
        ...
        console.info(
          "Number of visitors:",
          Math.floor(Math.random() * (40 - minimumRange + 1) + minimumRange)
        );
      }
      if (vehicle.vehicleType === "heavy") {
        const minimumRange = vehicle.visitors;
